...
const CreateDoc = () => {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      // Yer tutucuları bulma - Köşeli parantez içindeki içeriği yakalama
  ...
  // Herhangi bir şablonu yükleme fonksiyonu - bileşen içerisine taşındı
  const loadDefaultFile = async (templateName) => {
    try {
      setLoading(true);
      setDocxStatus('loading');
      
      // Eğer parametre verilmişse o şablonu, yoksa varsayılan dosyayı yükle
      const fileToLoad = templateName || fileName;
      
      const response = await fetch(`/${fileToLoad}`);
      const blob = await response.blob();
      const file = new File([blob], fileToLoad, { 
  ...
  // Örnek şablonlardan birini çağıran şablon seçme fonksiyonu
  const handleTemplateSelection = (templateName) => {
    loadDefaultFile(templateName);
  };
      ...
      <div className="py-12 bg-gray-50 dark:bg-dark-900 min-h-screen">
        <div className="container mx-auto px-4 sm:px-6 max-w-screen-xl">
          <div className="mb-10">
            ...
            {/* Yeni Dosya Yükleme Bileşeni */}
            <div className="mt-6 mb-8 bg-white dark:bg-dark-800 rounded-xl shadow-md p-6 border border-slate-200/70 dark:border-dark-700/70">
              <h2 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-primary-600 dark:text-primary-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                </svg>
                Kendi Belgenizi Yükleyin
              </h2>
              ...
              <div
                className={`relative flex flex-col items-center justify-center h-40 border-2 border-dashed rounded-lg transition-all duration-300 ${
                  dragActive
                    ...
                    : "border-gray-300 dark:border-dark-600 hover:bg-gray-50 dark:hover:bg-dark-700/50"
                }`}
              ...
              >
                ...
                <div className="flex flex-col items-center text-center p-4">
                  <svg xmlns="http://www.w3.org/2000/svg"
                    className={`h-10 w-10 mb-3 ${dragActive ? "text-primary-600 dark:text-primary-400" : "text-gray-400 dark:text-gray-500"}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                  ...
                  <p className="text-base font-medium text-gray-700 dark:text-gray-300 mb-1">
                    {dragActive ? "Dosyayı buraya bırakın" : "DOCX dosyanızı sürükleyip bırakın veya"}
                  </p>
                  ...
                  {!dragActive && (
                    <button
                      ...
                      className="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-lg text-sm transition-colors inline-flex items-center"
                    >
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Sadece DOCX dosyaları desteklenmektedir</p>
                </div>

                {docxStatus === 'ready' && (
                  <div className="absolute top-2 right-2 inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300">
              ...
              {/* Örnek Şablon Açıklaması */}
              <div className="mt-4 p-3.5 bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 dark:border-blue-400 text-blue-800 dark:text-blue-300 text-sm">
                <p className="flex items-start">
                  <svg className="h-5 w-5 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <span>
                    <strong>İpucu:</strong> Doldurulabilir alanları belgenizde
                    <code className="mx-1 px-1.5 py-0.5 bg-blue-100 dark:bg-blue-800/50 rounded font-mono">[Alan Adı]</code>
                    formatında belirtin. Örneğin:
                    <code className="ml-1 px-1.5 py-0.5 bg-blue-100 dark:bg-blue-800/50 rounded font-mono">[Müşteri Adı]</code>
                  </span>
                </p>
              </div>
              ...
              {/* Örnek Şablonlar */}
              <div className="mt-4">
                <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Veya hazır şablonlarımızı kullanın:</h3>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <button
                    onClick={() => handleTemplateSelection("cagri.docx")}
                  ...
                  >
                    <svg className="w-6 h-6 text-blue-600 dark:text-blue-400 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                    <div className="text-left">
                      <p className="font-medium text-gray-800 dark:text-white text-sm">Çağrı Dilekçesi</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">Mahkeme çağrı dilekçesi şablonu</p>
                    </div>
                  </button>

                  <button
                    onClick={() => handleTemplateSelection("ihtarname.docx")}
                    className="flex items-center p-3 border border-gray-200 dark:border-dark-600 rounded-lg hover:bg-gray-50 dark:hover:bg-dark-700 transition-colors"
                  >
                    ...
                    <div className="text-left">
                      <p className="font-medium text-gray-800 dark:text-white text-sm">İhtarname</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">İhtarname örneği</p>
                    </div>
                  </button>

                  <button
                    onClick={() => handleTemplateSelection("sozlesme.docx")}
                    className="flex items-center p-3 border border-gray-200 dark:border-dark-600 rounded-lg hover:bg-gray-50 dark:hover:bg-dark-700 transition-colors"
                  >
                    ...
                    <div className="text-left">
                      <p className="font-medium text-gray-800 dark:text-white text-sm">Sözleşme</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">Genel sözleşme taslağı</p>
                    </div>
                  </button>
                </div>
              </div>
